...
def recommend_never_seen(user, recommendations):
    ...
    for item in maps.items_never_seen:
        ...
        shrink = map(lambda x: float(maps.get_user_feature_evaluation_count(user, x)) / len(
            maps.get_user_evaluation_list(user)), features)
