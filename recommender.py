...
class Recommender:
    ...
    def make_top_tag_recommendations(self, active_playlist, target_tracks=[], recommendations=[], knn=5, ensemble=0):
        ...
        * lastly the position of the track in top included
        ...
        for track in target_tracks: # make the actual recommendation
            ...
            top_tracks.append([track, value_c * sum([self.db.get_item_similarities_alt(track,j) for j in active_tracks]) * value_d * value_a])
    ...
    def ensemble_tfidf_naive_recommendations(self, playlist):
        ...
        possible_recommendations = [[item, a * recommendations1[item] + c * recommendations3[item] + b * recommendations2[item] + d * recommendations4[item] + g * recommendations7[item]] for item in target_tracks]
        possible_recommendations.sort(key=itemgetter(1), reverse=True)
