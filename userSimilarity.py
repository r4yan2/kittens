...
import csv
import math
from maps import get_train_user_set, get_avg_user_rating, get_num_items, get_user_evaluation_list, get_num_users, get_item_evaluators_list, get_evaluation
from user_based import pearson_user_based_correlation 

def get_user_similarities(userI):
    ...
    blacklist = [userI] # will help to filter out user with no similairty
    ...
    return sorted(similarities, key=lambda x: x[2], reverse=True)[:5] #10 is choosen based on the number which the coder has dreamed last night
